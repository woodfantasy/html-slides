...
**共享特性**：横向翻页（← → / Space / 滚轮 / 触屏）、ESC 索引、S 键 Speaker View、B 键低功耗、WebGL 背景、Lucide 图标、Anime.js v4 动效。
...
### Step 0 · 读场推断（Brief Inference）

**在问任何问题之前**，先根据用户输入推断一行 Design Read：

> "本次推断：**[场景]** 的 **[受众]** 分享，偏 **[风格关键词]** 语言，推荐 **[风格名]** + **[配色方案]**"
...
### Step 1 · 需求澄清（仅 Step 0 无法推断时）

**多数情况 Step 0 已足够**，只在以下情况展开 7 问：用户要求详细对齐、内容极复杂、有硬约束未表达。
...
5. 读 `references/animation-recipes.md` 选择动效配方
6. 生成后运行 `node scripts/validate.mjs`
7. 读 `references/checklist.md` 逐项自检
